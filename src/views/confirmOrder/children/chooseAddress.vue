<template>
  <div class="rating_page">
    ...
    >
      <img
        src="../../../assets/images/add_address.png"
        height="24"
        width="24"
      />
      <span>新增收货地址</span>
        ...
        <ul class="deliverable_address">
          ...
          >
            ...
            <Icon
              :class="[
                'choosed_address',
                { default_address: defaultIndex == index },
              ]"
        ...
        <section id="out_delivery" v-if="deliverdisable.length">
          ...
          <ul class="deliverable_address">
            <li v-for="(item, index) in deliverdisable">
              ...
              <Icon class="choosed_address" type="ios-checkmark-circle" />
              <div>
                <header>
                  <span>{{ item.name }}</span>
                  <span>{{ item.sex == 1 ? "先生" : "女士" }}</span>
...
export default {
  ...
  props: [],
  computed: {
    ...mapState(["userInfo", "addressIndex", "newAddress", "addressList"]),
    //选择地址
  ...
  methods: {
    ...mapMutations(["CHOOSE_ADDRESS", "SAVE_ADDRESSlIST"]),
    //初始化信息
    async initData() {
      //   this.addressList = [];
      this.deliverable = [];
      ...
      if (this.userInfo && this.userInfo.user_id) {
        if (!(this.addressList instanceof Array)) {
          this.loadShow = true;
          const addressRes = await getAddressList(this.userInfo.user_id);
          if (addressRes.length !== undefined) {
            this.SAVE_ADDRESSlIST([...addressRes]);
            this.loadShow = false;
          } else {
            this.alertText = "获取失败,未知错误";
            this.showAlert = true;
            this.loadShow = false;
          }
        }
  ...
  watch: {
    ...
    newAddress: function(value) {
      ...
      // 存入vuex
      // 添加在对应列表中
      const addressRes = getAddressList(this.userInfo.user_id).then((res) => {
        if (res instanceof Array) {
          this.SAVE_ADDRESSlIST([...res]);
          res.forEach((item) => {
            if (item.is_deliverable) {
              this.deliverable.push(item);
            } else {
